...
while {!BIS_WL_missionEnd} do {
	...
	private _timeout = (120 * (_sideCnt/10)) max 30;
	sleep _timeout;
	if (missionNamespace getVariable ["WL2_manLost", false]) then {
		missionNamespace setVariable ["WL2_manLost", false];
	} else {
		BIS_WL_matesAvailable = (BIS_WL_matesAvailable + 1) min (missionNamespace getVariable [format ["BIS_WL_maxSubordinates_%1", _side], 2]);
	};
